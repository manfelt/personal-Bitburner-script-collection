...
export async function main(ns) {
  ...
  var hakkelevel = ns.getHackingLevel();
  var homeRam = ns.getServerMaxRam(localMachine);
  ...
  var srlist = ['run4theh111z', 'n00dles', 'foodnstuff', 'sigma-cosmetics',
    ...
    'netlink', 'avmnite-02h', 'titan-labs', 'johnson-ortho',
  ...
  // TODO this doesn't actually do anything, it needs to be fixed.
  function reconfigure(hakkelevel, homeRam, machine) {
    if (ns.getHackingLevel() > hakkelevel) {
      window['hakkelevel'] = ns.getHackingLevel();
      window['homeRam'] = ns.getServerMaxRam(localMachine);
      window['available'] = getAvailableSrv(srlist);
      window['bank'] = findBank(available);
      window['treds'] = getTreds(localMachine);
      window['curScript'] = "unset";
    } else if (ns.getServerMaxRam(machine) > homeRam) {
      window['hakkelevel'] = ns.getHackingLevel();
      window['homeRam'] = ns.getServerMaxRam(localMachine);
      window['available'] = getAvailableSrv(srlist);
      window['bank'] = findBank(available);
  ...
  while (true) {
    ...
    reconfigure(hakkelevel, homeRam, localMachine)
